...
app.post("/players/", async (request, response) => {
  ...
  const result = await db.run(addPlayerDetails);
...
app.put("/players/:playerId/", async (request, response) => {
  ...
  const playerDetails = request.body;
  const { playerName, jerseyNumber, role } = playerDetails;
  const addPlayerDetails = `UPDATE cricket_team SET player_name = '${playerName}' , jersey_number = ${jerseyNumber} , role = '${role}' WHERE player_id = ${playerId}`;
